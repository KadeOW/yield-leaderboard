...
// ─── Curated example strategies (shown when no on-chain data available) ───────

const CURATED: DetectedStrategy[] = [
  {
    name: 'Yield Loop: Avon → Kumbaya',
    description: 'Deposit USDM into Avon to earn vault yield and receive USDMy. Re-deploy USDMy as liquidity in a Kumbaya pool to stack LP fees on top of vault yield.',
    isLoop: true,
  ...
  },
  {
    ...
    description: 'Deposit USDM into Avon, then use the USDMy shares as one side of a concentrated LP on Prism. Your principal earns vault yield while the LP earns swap fees.',
    isLoop: true,
    complexity: 'Intermediate',
    baseAPY: 8,
    bonusAPY: 25,
  ...
  },
  {
    ...
    description: 'Split USDMy across both Kumbaya and Prism to diversify LP risk while maximising fee capture. Vault yield continues compounding underneath.',
    isLoop: true,
    complexity: 'Advanced',
    baseAPY: 8,
    bonusAPY: 85,
  ...
  },
  {
    ...
    description: 'Provide WETH-paired liquidity directly on Kumbaya without the vault layer. Higher capital efficiency if you want direct ETH exposure.',
    isLoop: false,
    complexity: 'Simple',
    baseAPY: 80,
    bonusAPY: 0,
    ...
    <div className="flex items-center gap-1 flex-wrap">
      {steps.map((step, i) => (
        <div key={i} className="flex items-center gap-1">
          {/* Token pill */}
          <div className="text-xs px-2 py-1 rounded-md bg-white/5 text-gray-300 font-mono">
            {step.inputToken}
          </div>
          ...
          <span className="text-gray-600">→</span>
          {/* Only show output token after last step */}
          {i === steps.length - 1 && (
            <div className="text-xs px-2 py-1 rounded-md bg-accent/10 text-accent font-mono border border-accent/20">
              {step.outputToken}
            </div>
          )}
        </div>
        ...
        <div key={step.stepNumber} className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Earn <span className="text-accent">{formatAPY(step.apy)}</span> · Input: {step.inputToken} · Output: {step.outputToken}
            </p>
          </div>
          <a
            href={step.url}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 flex-wrap mb-3">
        {isOwn && (
          <span className="text-xs px-2 py-0.5 rounded-full bg-accent/15 border border-accent/30 text-accent">
            Your Strategy
          </span>
        )}
        {strategy.isLoop && (
        ...
        <span className={`text-xs px-2 py-0.5 rounded-full border ${COMPLEXITY_COLORS[strategy.complexity]}`}>
          {strategy.complexity}
        </span>
        {walletAddress && (
          <span className="text-xs font-mono text-gray-600 ml-auto">{truncateAddress(walletAddress)}</span>
        )}
      </div>
      ...
      <div className="flex items-center gap-4 mb-4">
        <div>
          ...
          <p className="text-lg font-bold text-white">{formatAPY(strategy.baseAPY)}</p>
        </div>
        {strategy.bonusAPY > 0 && (
          <>
            <div className="text-gray-600 text-lg">+</div>
            <div>
              <p className="text-xs text-gray-500">LP Bonus</p>
              <p className="text-lg font-bold text-accent">{formatAPY(strategy.bonusAPY)}</p>
            </div>
            <div className="text-gray-600 text-lg">=</div>
          </>
        )}
        <div>
          ...
          <p className="text-xl font-bold text-accent">{formatAPY(strategy.totalAPY)}</p>
        </div>
        {strategy.totalValue > 0 && (
          <div className="ml-auto text-right">
            <p className="text-xs text-gray-500">Value</p>
            <p className="text-sm font-semibold text-white">{formatUSD(strategy.totalValue)}</p>
          </div>
        )}
      </div>
    ...
    <div className="card border-accent/10 bg-accent/[0.02]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div>
          ...
          <p className="text-sm font-medium text-white mb-1">Deposit into a vault</p>
          <p className="text-xs text-gray-500 leading-relaxed">
            Put USDM into Avon. You receive USDMy — a yield-bearing token that grows in value over time as the vault earns.
          </p>
        </div>
        <div>
          <div className="text-2xl mb-2">2️⃣</div>
          <p className="text-sm font-medium text-white mb-1">Re-deploy the receipt token</p>
          <p className="text-xs text-gray-500 leading-relaxed">
            Use USDMy as liquidity on Prism or Kumbaya. Your LP position earns swap fees <em>on top of</em> the Avon vault yield still compounding underneath.
          </p>
        </div>
        <div>
          <div className="text-2xl mb-2">3️⃣</div>
          <p className="text-sm font-medium text-white mb-1">Stack the yields</p>
          <p className="text-xs text-gray-500 leading-relaxed">
            Both layers pay simultaneously. The vault yield increases the value of USDMy, which means your LP position value also grows — compounding your returns.
          </p>
        </div>
      </div>
    </div>
...
export default function StrategiesPage() {
  ...
  // Show on-chain discovered wallets; fall back to curated examples
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in space-y-10">
      ...
      <div>
        <div className="flex items-center gap-2 mb-2">
          <h1 className="text-2xl font-bold text-white">Strategy Discovery</h1>
          <span className="text-xs px-2 py-0.5 rounded-full bg-accent/10 border border-accent/20 text-accent">Live on-chain</span>
        </div>
        <p className="text-sm text-gray-500 max-w-2xl">
          Looping strategies stack yield across multiple protocols. Deposit into a vault, receive a yield-bearing token, then re-deploy it as LP liquidity — earning both vault yield and swap fees simultaneously.
        </p>
      </div>
      ...
      <div>
        ...
        {isConnected && !myLoading && myStrategy && (
          <StrategyCard strategy={myStrategy} isOwn />
        )}
        {isConnected && !myLoading && !myStrategy && (
      ...
      <div>
        <div className="flex items-center gap-3 mb-4">
          ...
          {scanLoading && (
            <span className="text-xs text-gray-500 animate-pulse">Scanning chain…</span>
          )}
          {!scanLoading && communityStrategies && (
            <span className="text-xs text-gray-500">{communityStrategies.length} wallets found</span>
          )}
          {!scanLoading && !communityStrategies && (
            <span className="text-xs text-gray-600">No loop wallets found on-chain yet — showing curated examples</span>
          )}
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {communityStrategies
            ? communityStrategies.map((d) => (
                <StrategyCard key={d.address} strategy={d.strategy} walletAddress={d.address} />
              ))
            : CURATED.map((s) => (
