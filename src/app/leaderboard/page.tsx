...
function ActivityRow({ event }: { event: ActivityEvent }) {
  ...
  return (
    <div className="flex items-center justify-between py-2 border-b border-border last:border-0">
      ...
      <p className="text-xs text-gray-500">{event.timestamp ? formatPositionAge(event.timestamp) : '—'}</p>
    </div>
  );
}
        ...
        <div className="mt-4 pt-4 border-t border-border">
          ...
          {tab === 'positions' && (
            <>
              ...
              {!posLoading && (!positions || positions.length === 0) && (
                <p className="text-sm text-gray-500 text-center py-6">No positions found.</p>
              )}
            </>
          )}
  ...
  return (
    <form onSubmit={handleSubmit} className="flex gap-2 mb-6">
      <input
        ...
        placeholder="0x... paste any wallet address to follow"
      ...
      />
      <button type="submit" className="btn-primary px-5 py-2 text-sm">Follow</button>
...
export default function LeaderboardPage() {
  const [tab, setTab] = useState<'leaderboard' | 'watching'>('leaderboard');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Top yield earners on MegaETH · Follow wallets to track their positions</p>
        </div>
      </div>
      ...
      <div className="flex gap-1 mb-6 border-b border-border">
        ...
        >
          Watching
          {addresses.length > 0 && (
            <span className="ml-1.5 text-xs px-1.5 py-0.5 rounded-full bg-accent/15 text-accent">
              {addresses.length}
            </span>
          )}
        </button>
        ...
        <>
          {isLoading ? (
            <div className="space-y-2">
              {[...Array(10)].map((_, i) => <div key={i} className="h-12 skeleton rounded-lg" />)}
            </div>
          ) : entries ? (
            <LeaderboardTable
              entries={entries}
              watchedAddresses={watchedSet}
        ...
        <>
          ...
          {addresses.length === 0 ? (
            <div className="card text-center py-16">
              ...
              <p className="font-medium text-white mb-2">Not following anyone yet</p>
              <p className="text-sm text-gray-500 max-w-sm mx-auto">
                Paste a wallet address above, or click <span className="text-accent">+ Follow</span> on any leaderboard entry.
              </p>
            </div>
          ) : (
